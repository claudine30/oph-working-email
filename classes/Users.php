...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		foreach($_POST as $k => $v){
			if(in_array($k,array('studID', 'firstname','middlename','lastname','username','type', 'gender', 'dob', 'contact', 'course'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if($resp['status'] == 1){
			$data="";
			foreach($_POST as $k => $v){
				if(!in_array($k,array('studID', 'id','firstname','middlename','lastname','username','password','type', 'gender', 'dob',  'contact', 'course', 'oldpassword'))){
					if(!empty($data)) $data .=", ";
